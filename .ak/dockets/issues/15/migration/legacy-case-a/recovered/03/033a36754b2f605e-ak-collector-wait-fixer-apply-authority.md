...
Apply the authorized issue #3 plan on a strict forward descendant of a817975: add the private 300_000 ms term at createCollectorLedger().wait(), align only the existing role wait wording, and add the specified real-seam red/green tests including deterministic exact fake-clock setup for the PR4 shape and a real observe completing before deadline. Preserve schema maximum, near-cutoff and post-cutoff laws, records, abort behavior, and excluded surfaces; run typecheck, isolated-HOME tests, pack dry-run, and diff check.
