# Approved final Collector repair
...
## Binding Judge construction order

Plan gate passed; construction is authorized from clean HEAD 3bec03f84b1bee6a5f7d7c7068028b3c2849116f. In src/collector-ledger.ts, retain the existing single full-surface retry after first-bracket prIdentity drift, then compare the retry prInitial and prTerminal identities. If retry drift persists, throw through the existing observe fatal path before normalization, evidence storage, marker recovery, snapshot push, latestCompleteSnapshotId binding, or model-view return; add no further retry mechanism. In test/collector-ledger.test.ts, add the real-ledger OPEN/A→OPEN/B then OPEN/C→OPEN/D regression with non-empty evidence and assert exactly four pull reads, rejection plus fatal state, zero evidence, zero snapshots, undefined latestCompleteSnapshotId, and failed receipt materialization. Preserve the A→B then B→B success control binding terminal B. Run typecheck, the empty-HOME full suite, pack dry-run, and git diff --check, then make a forward commit without amend.
