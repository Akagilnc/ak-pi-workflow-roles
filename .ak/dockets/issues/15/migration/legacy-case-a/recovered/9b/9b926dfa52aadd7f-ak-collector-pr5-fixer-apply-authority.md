# Approved Collector first-order repair plan
...
Plan posture is approved; proceed to Apply at clean 9cc03df. Implement the stated existing-owner repairs R1–R11 without changing authority: R1 terminal precedence for both missing/unavailable; R2 every valid cite same-leg, final-snapshot, qualifying and rebound to qualifying proofs; R3 target unavailability bound to evidence establishment HEAD; R4 delete selective embedding and make every included snapshot evidence ID resolve; R5 preserve known review-version authoritative time and project modelView from canonical stored snapshot records; R6 preserve all three null-author surfaces while preventing qualification; R7 retain originalLine in review-comment version identity/record and use current-line-then-original-line report fallback; R8 retain only login/stable ID for authenticated-user evidence; R9 include updatedAt in the existing two-bracket/full-retry/fail-closed identity; R10 enforce one incremental retained-payload budget across observe pagination while retaining the exact final 8 MiB normalized check; R11 thread the one observe AbortSignal through ledger, GET transport, and owned gh child with single settlement/listener cleanup and no snapshot certification on abort. For each root, first add the plan’s focused red that fails on 9cc03df, then make it green; replace the two contradictory receipt tests rather than preserving parallel behavior. Apply fixture obligations: use GitHub wire-format fixtures with snake_case fields and literal user:null at transport normalization seams; use normalized camelCase fixtures only behind the fake transport for ledger/receipt seams; use actual GhApiResponse JSON-page and Link-header syntax for pagination; use an owned-child event fixture for cancellation. Do not add quasi-code to the plan. Preserve and rerun the existing regressions for a817975 repeated-drift fail-closed, 6fc54e1 five-minute single-wait cap, and 9cc03df provider-visible JSON.stringify(modelView); add no Soul/schema churn, duplicate classifier, subset path, metadata poll, or second AbortController. Finish on a new forward commit with npm test, npm run typecheck, git diff --check, and a clean worktree.
