...
AUTHORIZED FOR APPLY on clean c86c2a2: implement the plan as written. Replace `paginate()`’s per-call raw `retainedBytes` cap with one ledger-owned normalized-record budget per `fetchObserveSurfaces` attempt; invoke the page retain hook before aggregate append/next-page fetch across reviews, issue comments, and review comments; remove the obsolete collector-github size import; preserve Link pagination and the exact final history/window/`measureNormalizedBytes` gate. Add the real-path cumulative cross-surface rejection test, the normalized intra-surface multi-page test, fake-transport hook parity, and run collector ledger/GitHub tests plus typecheck. The crossing test must assert every applicable later surface call and the terminal PR read do not run, fatal rejection occurs, and no snapshot is committed.
