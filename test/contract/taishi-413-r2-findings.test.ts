...
import {
  isSyntheticTaishiBookKey,
} from "../../src/taishi-book-key.ts";
import {
...
import { taishiIssuePagePath } from "../../src/taishi-page.ts";

const ABSENT_METRIC = { status: "absent" as const };

function withTempLedgerHome(): string {
  const home = mkdtempSync(join(tmpdir(), "taishi-413r2-home-"));
  mkdirSync(join(home, ".ak-roles", "taishi"), { recursive: true });
  return home;
}
...
test("U1: library-index read boundary rejects null/non-object/rows-not-array with the file path", async () => {
  ...
  try {
    const indexPath = taishiLibraryIndexPath(join(home, ".ak-roles"));
    ...
    await writeFile(
      indexPath,
      `${JSON.stringify({ kind: "taishi-library-index", rows: [] })}\n`,
      "utf8",
    );
    const page = await readTaishiLibraryIndexPage(join(home, ".ak-roles"));
...
test("U3: isSyntheticTaishiBookKey — root:<absolute path> is synthetic; real basename root:foo is not", () => {
  ...
  assert.equal(isSyntheticTaishiBookKey("root:foo"), false);
  assert.equal(isSyntheticTaishiBookKey("plain-book"), false);
  // Existing synthetic keys keep their meaning: root: + absolute path identity.
  assert.equal(isSyntheticTaishiBookKey(`root:${physicalPathIdentity("/taishi-fixture/u3")}`), true);
});
...
test("U3: real book basename root:foo keeps its book scope through cohort cache-miss recompute", async () => {
  const home = await mkdtemp(join(tmpdir(), "taishi-413r2-e2e-"));
  const previousHome = process.env.HOME;
  process.env.HOME = home;
  try {
    const ledgerHome = join(home, ".ak-roles");
    // Real Git repository whose book key (common-dir host basename) is literally root:foo.
    const repoParent = mkdtempSync(join(tmpdir(), "taishi-413r2-repos-"));
