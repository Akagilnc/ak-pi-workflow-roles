/**
 * #399 taishi book-scope — three defects on the analysis seat (owner-ratified).
 *
...
async function countTaishiFiles(home: string): Promise<number> {
  const root = join(home, ".ak-roles", "taishi");
  let count = 0;
...
async function withBookScopeWorld<T>(
  ...
  try {
    ...
    worktreeRoot = join(await mkdtemp(join(tmpdir(), "taishi-399-wt1-")), "wt");
    worktree2Root = join(await mkdtemp(join(tmpdir(), "taishi-399-wt2-")), "wt");
...
// D1
test("D1 taishi #399 --ticket filters strictly; empty of unbound; != bare set", async () => {
  await withBookScopeWorld(async ({ home, mainRoot }) => {
    const previousCwd = process.cwd();
    process.chdir(mainRoot);
    try {
      const bare = captureIo();
      const bareResult = await runAkRole(["taishi"], { packageRoot, home, io: bare.io });
...
// D2
test("D2 taishi #399 bare sees main+2 worktree runs; --project-root rejected", async () => {
...
// D3
test("D3 taishi #399 --ticket without library-index: live book compute", async () => {
  await withBookScopeWorld(async ({ home, mainRoot }) => {
    await assert.rejects(
      () =>
        import("node:fs/promises").then((fs) =>
          fs.stat(taishiLibraryIndexPath(join(home, ".ak-roles"))),
        ),
...
// D4
test("D4 taishi #399 non-git cwd bare: nonzero + must-enter-repo; taishi file count stable", async () => {
  const home = await mkdtemp(join(tmpdir(), "taishi-399-nongit-home-"));
  const nonGit = await mkdtemp(join(tmpdir(), "taishi-399-nongit-cwd-"));
  const previousHome = process.env.HOME;
  process.env.HOME = home;
  const previousCwd = process.cwd();
  try {
    await mkdir(join(home, ".ak-roles", "taishi"), { recursive: true });
    const before = await countTaishiFiles(home);
    ...
    const after = await countTaishiFiles(home);
...
// D5
test("D5 taishi #399 two books ticket 181: pages distinct by book identity", async () => {
  const home = await mkdtemp(join(tmpdir(), "taishi-399-d5-home-"));
  const repoA = await mkdtemp(join(tmpdir(), "taishi-399-d5-a-"));
  const repoB = await mkdtemp(join(tmpdir(), "taishi-399-d5-b-"));
  const previousHome = process.env.HOME;
  process.env.HOME = home;
  const previousCwd = process.cwd();
  try {
    ...
    const a = captureIo();
    assert.equal(
      (await runAkRole(["taishi", "--ticket", String(TICKET_CROSS)], { packageRoot, home, io: a.io }))
        .exitCode,
      0,
      a.stderr.join(""),
    );
    const pageA = (JSON.parse(a.stdout.join("")) as { page: TaishiIssueMetricsPage; pagePath: string });

    process.chdir(repoB);
    const b = captureIo();
    assert.equal(
      (await runAkRole(["taishi", "--ticket", String(TICKET_CROSS)], { packageRoot, home, io: b.io }))
        .exitCode,
      0,
      b.stderr.join(""),
    );
    const pageB = (JSON.parse(b.stdout.join("")) as { page: TaishiIssueMetricsPage; pagePath: string });
    ...
    assert.equal(
      pageA.pagePath,
      taishiIssuePagePath(join(home, ".ak-roles"), {
...
// D6
test("D6 taishi #399 unmatched ticket: honest empty page, no full-book fallback", async () => {
  await withBookScopeWorld(async ({ home, mainRoot }) => {
    const previousCwd = process.cwd();
    process.chdir(mainRoot);
    try {
...
// D7
test("D7 taishi #399 damaged invocation/session stays unreadable (loud exclusion)", async () => {
...
test("taishi #399 library bookKey scope: whole book includes worktree runs", async () => {
  await withBookScopeWorld(async ({ mainRoot, bookKey }) => {
    const result = await runTaishi({
      mode: "issue",
      bookKey,
