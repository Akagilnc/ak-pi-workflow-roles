...
import { DOCTOR_COMMITTED_IDENTITY_ERROR_CODE, DoctorCommittedIdentityError, DoctorEvidenceStore, statsLineEvidenceBytes, validateDoctorEvidenceIndex, validateDoctorOutput, validateRecordedDoctorOutput } from "../src/doctor-contracts.ts";
...
const biteIdentity = entry("bite", "receipt", { blocked: true }, true);
const committedIdentities = new Map([["bite", { sha256: biteIdentity.sha256, byteLength: biteIdentity.byteLength }]]);
function validateIndex(value: unknown) { return validateDoctorEvidenceIndex(value, committedIdentities); }
function index() { return validateIndex({ version: 1, repository: "ak/repo", targetCommit: "a".repeat(40), catalog: [{ key: target, kind: "gate", active: true }], populations: [{ id: "bounded", targetKey: target, eligibleEvidenceIds: ["bite"] }], evidence: [biteIdentity, entry("stats-1", "statsLine", line(1)), entry("stats-2", "statsLine", line(2))] }); }
...
test("Doctor intake rejects forbidden evidence kinds, digest drift, duplicate identities, and malformed StatsLines", () => {
  ...
  assert.throws(() => validateIndex({ ...base, evidence: [...base.evidence, entry("session", "session", {}, false)] }), /Forbidden or unknown/);
  assert.throws(() => validateIndex({ ...base, evidence: [base.evidence[0], { ...base.evidence[1], data: line(9) }, ...base.evidence.slice(2)] }), /digest mismatch/);
  assert.throws(() => validateIndex({ ...base, evidence: [...base.evidence, base.evidence[0]] }), /Duplicate evidence/);
...
test("direct validation cannot canonical-hash a self-declared committed claim", () => {
  ...
  assert.throws(
    () => validateDoctorEvidenceIndex(base),
    (error) => error instanceof DoctorCommittedIdentityError && error.code === DOCTOR_COMMITTED_IDENTITY_ERROR_CODE && error.evidenceId === "bite",
  );
  const normalizedOnly = { ...base, populations: [{ ...base.populations[0], eligibleEvidenceIds: [] }], evidence: base.evidence.filter((item) => item.kind === "statsLine") };
  assert.equal(validateDoctorEvidenceIndex(normalizedOnly).evidence.length, 2);
});
