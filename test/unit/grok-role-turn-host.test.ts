...
`);
    ...
    // #617 DK-4: same-host Grok resume reuses native ACP session via session/load.
    assert.equal(sessionCalls.some(([method]) => method === "session/load"), true);
  } finally {
...
/** #617 DK-4: prior Pi native resource only when hostTransition says so. */
test("grok resume injects prior Pi native resource only on typed host transition", async () => {
  const root = await mkdtemp(join(tmpdir(), "ak-grok-host-transition-"));
  try {
    const runDirectory = join(root, "run");
    const sessionDir = join(runDirectory, "session");
    await mkdir(sessionDir, { recursive: true });
    const marker = "unit-pi-prior-marker-617";
    await writeFile(
      join(sessionDir, "session.jsonl"),
      `${JSON.stringify({ type: "session", id: "s" })}\n${JSON.stringify({ type: "message", message: { role: "user", content: marker } })}\n`,
      "utf8",
    );

    async function runWithTransition(hostTransition: { previousHost: string } | undefined) {
      ...
      const base = turnRequest({ runDirectory, continuation: { kind: "resume", prompt: "go" } });
      const result = await host.executeTurn(
        hostTransition === undefined ? base : { ...base, hostTransition },
      );
      assert.equal(result.code, 0);
      return prompts;
    }

    function priorResource(prompts: Array<Readonly<Record<string, unknown>>>) {
      const parts = prompts[0]?.prompt;
      if (!Array.isArray(parts)) return undefined;
      return parts.find((part) =>
        typeof part === "object"
        && part !== null
        && (part as { type?: unknown }).type === "resource"
        && (part as { resource?: { mimeType?: unknown } }).resource?.mimeType === "application/x-ak-prior-native",
      ) as { resource?: { text?: unknown; uri?: unknown } } | undefined;
    }

    const withTransition = await runWithTransition({ previousHost: "pi" });
    const injected = priorResource(withTransition);
    assert.equal(injected?.resource?.uri, "ak-role:prior-native/pi");
    assert.ok(String(injected?.resource?.text ?? "").includes(marker));

    const without = await runWithTransition(undefined);
    assert.equal(priorResource(without), undefined, "no hostTransition ⇒ no prior native resource");
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});
