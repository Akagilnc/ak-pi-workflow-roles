...
const ACCEPTED_DETAILS_BY_TOOL: ReadonlyMap<string, unknown> = new Map<string, unknown>([
  ...
  [COUNTERSIGN_OUTPUT_TOOL_NAME, { countersignStatus: "converged", findings: [] }],
]);
...
function extractForTool(
  ...
  switch (toolName) {
    ...
    case COUNTERSIGN_OUTPUT_TOOL_NAME:
      return extractCountersignRoleOutcome(entries as never);
    default:
...
test("registry public extractors and settlement share one closed terminal classifier", () => {
  ...
  assert.deepEqual(
    [...rolesSeen].sort(),
    ["coder", "collector", "countersign", "doctor", "fixer", "judge", "merger", "notary", "reviewer"],
  );
