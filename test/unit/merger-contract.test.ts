...
test("Merger input admits empty path materials and does not gate on OID/digest/scope", () => {
  const emptyPaths = validateMergerInput({
    ...valid(),
    targetObjectId: "",
    sourceObjectId: "",
...
test("Merger terminal leaves discriminate completed|escalate without attempt/OID gates", () => {
  const completed = validateMergerOutput({
    status: "completed",
    attemptId: "any",
    report: "resolved",
    mergeCommitId: "not-an-oid",
  });
  assert.equal(completed.status, "completed");
  const escalated = validateMergerOutput({
    status: "escalate",
    attemptId: "any",
    diagnosis: "no live merge",
    report: "nothing to do",
  });
  assert.equal(escalated.status, "escalate");
  assert.throws(
    () => validateMergerOutput({ status: "other", attemptId: "x", report: "x" }),
    /执行判别/,
  );
});
