...
const ISSUE_203_ROOT = "/analyst-fixture/c2-issue-203";
...
function absentRef(issueNumber: number) {
  return {
    bookKey: `root:${physicalPathIdentity(`/analyst-fixture/c2-absent-${issueNumber}`)}`,
    issueNumber,
  };
}
...
const present = (value: number): AnalystCohortOptionalMetric => ({
  status: "present",
  value,
});
...
async function withBusinessRepo<T>(fn: (repo: string) => Promise<T>): Promise<T> {
  const businessRepo = await mkdtemp(worktreeTempPrefix("analyst-c2-business-"));
  try {
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  const home = await mkdtemp(worktreeTempPrefix("analyst-c2-home-"));
  try {
...
function roleStats(
  ...
  role: string,
): AnalystCohortRoleStats | undefined {
