...
test("#636 continuous auditor volume: parent A must not absorb B-only provider failure", async () => {
  await withTempHome(async (home) => {
    const sessionDir = join(home, "session");
    const sessionFile = join(sessionDir, "parent.jsonl");
    const childDir = join(sessionDir, "auditor-roles");
    await mkdir(childDir, { recursive: true });
    // Parent session only has attempt A as the latest user turn.
    await writeFile(
      sessionFile,
      [
        { type: "session", id: "parent-session" },
    ...
    // Continuous ticket-seat volume: A succeeds, later B fails. Matching A must not
    // read B's provider stop (judge r1 binding probe).
    await writeFile(
    ...
    assert.equal(
      ...
      undefined,
      "parent A interval must not project B-only failure",
    );
  });
});
