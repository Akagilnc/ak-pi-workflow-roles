...
// #858 C1: call-local boundary capture at public resume must settle through the
// existing controlled-failure authority (typed session + error/evidence artifacts),
// not escape as a bare SyntaxError outside terminal presentation.
test("resume with malformed bound session settles typed session failure at entry capture", async () => {
  await withTempHome(async (home) => {
    const project = join(home, "proj-resume-malformed-boundary");
    await mkdir(project, { recursive: true });
    seedGitProject(project);
    const runId = "run-resume-malformed-boundary-001";
    let sessionFile = "";

    const { io: io1, stdout: stdout1, stderr: stderr1 } = captureIo();
    const first = await runAkRole(
      ["judge", "--model", "test/caller-seat:high", "--project", project, "seed resumable run"],
      {
        packageRoot,
        home,
        cwd: project,
          ...
          piRunner: async (args) => {
            ...
            await writeFile(
              ...
              [
                ...
                { type: "message", id: "user-1", message: { role: "user", content: "seed" } },
                {
                  type: "message",
                  id: "asst-1",
                  message: {
                    role: "assistant",
                    stopReason: "error",
                    errorMessage: "seed provider stop",
            ...
            return { code: 1, stderr: "seed-fail\n", timedOut: false, args: [...args] };
          },
        }),
      },
    );
    await assertPublicFailureSettlement({ result: first, stdout: stdout1, stderr: stderr1 });
    assert.ok(sessionFile.length > 0, "first turn must bind a session file");

    // Corrupt the bound session before the next public call so entry capture reads it.
    ...
    assert.equal(resumeDispatched, 0, "unreadable bound session must settle before host dispatch");
    const { terminal, errorRef } = await assertPublicFailureSettlement({
      result: resumed,
      stdout: stdout2,
      stderr: stderr2,
