...
test("layer ① accepted pass/bounce exit 0 via public entry", async () => {
  await withTempHome(async (home) => {
    ...
    for (const [index, receipt] of receipts.entries()) {
      ...
      // #621: named findings stay on typed decisiveFacts (standalone seat).
      assert.deepEqual(
        result.terminal.roleOutcome.decisiveFacts.findings,
        receipt.findings,
        `receipt ${receipt.status} findings`,
      );
    }
  });
});
...
/**
 * #621 acceptance — standalone notary public entry carries bounce/pass + named
 * clause findings on typed terminal keys (not free-text scrape).
 * Scenario fixtures are the role leg's already-formed findings pointers.
 */
test("#621 standalone notary terminal retains named findings for bounce and pass", async () => {
  await withTempHome(async (home) => {
    const project = join(home, "project");
    await mkdir(project, { recursive: true });
    seedGitProject(project);
    ...
    for (const [index, scenario] of scenarios.entries()) {
      ...
      const result = await runAkRole(
        ["notary", "--source-run", sourceRunPath],
        {
          ...
          io,
          createRunId: () =>
            `01a06210-0000-7000-8000-${String(index).padStart(12, "0")}`,
          roleTurnHost: roleTurnHostFromLegacyPiRunner({
            packageRoot,
            principalAuthority: piDurablePrincipalAuthority,
            piRunner: scriptedNotarySession(scenario),
          }),
        },
      );
      assert.equal(result.exitCode, 0, `scenario ${index}`);
      assert.ok(result.terminal);
      assert.equal(result.terminal.roleOutcome.kind, "accepted");
      assert.equal(result.terminal.roleOutcome.status, scenario.status);
      assert.deepEqual(
        result.terminal.roleOutcome.decisiveFacts.findings,
        scenario.findings,
      );
      if (scenario.status === "bounce") {
        assert.equal(
          result.terminal.roleOutcome.decisiveFacts.disposition,
          "rewrite",
        );
      }
    }
