...
test("taishi public CLI --ticket path: live book compute matches runTaishi oracle", async () => {
    ...
    await withTempHome(async (home) => {
      ...
      try {
        ...
        const result = await runAkRole(
          ["taishi", "--ticket", String(TICKET_C4)],
          { packageRoot, home, io },
        );
...
test("taishi public CLI bare call: whole book from cwd git common-dir", async () => {
  await withBusinessRepo(async (repo) => {
    await withTempHome(async (home) => {
...
test("taishi public CLI non-git cwd bare: usage-class failure + zero taishi writes", async () => {
  await withTempHome(async (home) => {
    const ledgerHome = join(home, ".ak-roles");
    await mkdir(join(ledgerHome, "taishi"), { recursive: true });
    const before = await snapshotTaishiDir(ledgerHome);
    const nonGit = await mkdtemp(join(tmpdir(), "taishi-336-nongit-"));
    const previousCwd = process.cwd();
...
test("taishi ticket parse rejects unsafe integers and infinity-length digit strings", () => {
  ...
  assert.equal(parseTaishiArgv([]).query, "issue");
});
