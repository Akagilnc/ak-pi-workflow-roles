...
function harness(flag: unknown = "/input.json") {
  const flags = new Map<string, unknown>([["ak-merger-input", flag]]);
...
function context(id: string, args: Record<string, unknown>, calls = 1, abort = () => {}): ExtensionContext {
  const sessionManager = SessionManager.inMemory();
  const content = Array.from({ length: calls }, (_, i) => ({
    type: "toolCall" as const,
    id: i ? `sibling-${i}` : id,
    name: i ? "bash" : MERGER_OUTPUT_TOOL_NAME,
    arguments: i ? {} : args,
  }));
  ...
  return { cwd: process.cwd(), sessionManager, abort, mode: "json" } as unknown as ExtensionContext;
}

function setup(overrides: any = {}) {
  const h = harness();
  const runtime = createMergerRoleRuntime(
    createPiRoleHostAdapter(h.pi as unknown as ExtensionAPI).host,
    {
      loadSoul: async () => "MERGER LAW",
...
test("role extension activates Merger without Git state dependency", async () => {
  await withHermeticHome({ prefix: "ak-merger-bind-cwd-" }, async ({ home }) => {
    const h = harness();
    h.pi.getFlag = (name: string) =>
      name === "ak-role" ? "merger" : name === "ak-merger-input" ? "/input.json" : undefined;
...
test("Merger accepts one honest escalation without Git success verification", async () => {
  ...
  await assert.rejects(
    h.tools.get(MERGER_OUTPUT_TOOL_NAME).execute("again", args, undefined, undefined, context("again", args)),
  );
});
