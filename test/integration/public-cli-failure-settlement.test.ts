...
test("classifyPostAdmissionFailure retains typed causes without washing identity", () => {
  ...
  // Concurrent thrown leaves: primary identity + secondary code both retained
  // (not washed to AggregateError name/message alone).
  const primaryHost = new Error("host primary boom");
  (primaryHost as { code?: string }).code = "EPRIMARY";
  const writeFail = new Error("last-host write boom");
  (writeFail as { code?: string }).code = "EISDIR";
  const concurrent = classifyPostAdmissionFailure({
    timedOut: false,
    code: null,
    stderr: "",
    thrown: new AggregateError(
      [primaryHost, writeFail],
      "host turn and ticket-seat last-host write failed",
      { cause: primaryHost },
    ),
  });
  assert.equal(concurrent.cause, "unrecognized");
  assert.equal(concurrent.diagnostic, "host primary boom");
  assert.equal(concurrent.identity?.code, "EPRIMARY");
  assert.equal(concurrent.identity?.name, "Error");
  const concurrentLeaves = concurrent.details?.concurrentFailures;
  assert.ok(Array.isArray(concurrentLeaves));
  assert.equal(
    (concurrentLeaves as Array<{ identity?: { code?: string } }>)[0]?.identity
      ?.code,
    "EISDIR",
  );

  // Nested AggregateError flattens; undefined primary leaf stays unrecognized.
  const nestedCleanup = new Error("cleanup boom");
  (nestedCleanup as { code?: string }).code = "ECLEANUP";
  const nested = classifyPostAdmissionFailure({
    timedOut: false,
    code: null,
    stderr: "",
    thrown: new AggregateError(
      [
        new AggregateError([undefined, nestedCleanup], "body and cleanup failed", {
          cause: undefined,
        }),
        writeFail,
      ],
  ...
  assert.equal(nested.diagnostic, "undefined");
  assert.equal(nested.identity, undefined);
  const nestedLeaves = nested.details?.concurrentFailures as Array<{
    identity?: { code?: string };
  }>;
  assert.ok(Array.isArray(nestedLeaves));
  assert.equal(nestedLeaves.length, 2);
  assert.equal(nestedLeaves[0]?.identity?.code, "ECLEANUP");
  assert.equal(nestedLeaves[1]?.identity?.code, "EISDIR");

  // Single-leaf AggregateError projects the leaf, not the aggregate shell.
  const singleLeaf = classifyPostAdmissionFailure({
    timedOut: false,
    code: null,
    stderr: "",
    thrown: new AggregateError([primaryHost], "wrapped once", { cause: primaryHost }),
  });
  assert.equal(singleLeaf.diagnostic, "host primary boom");
  assert.equal(singleLeaf.identity?.code, "EPRIMARY");
  assert.equal(singleLeaf.details, undefined);
});
