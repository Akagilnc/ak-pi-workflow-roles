...
test("Doctor audit receives the frozen evidence index without replaying large session content", async () => {
  ...
  await audit({ soul: "Doctor law", patient, readRecord: [{ evidenceId: "review/session/live.jsonl", fullyRead: true }], output: { status: "refused", reason: "missing", missingEvidence: [{ need: "bytes", targetKeys: ["review"] }] } }, { context });
