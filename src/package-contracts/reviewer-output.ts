...
export function validateRuntimeReviewerReceipt(output: unknown): RuntimeReviewerReceiptV2 {
  ...
  if (hasBatch !== Object.hasOwn(output.identities, "construction") || hasBatch !== Object.hasOwn(output.identities, "target") ||
      (hasBatch && (typeof output.batchIdentity !== "string" || output.batchIdentity.length === 0 || !isRecord(output.identities.construction) || !isRecord(output.identities.target)))) throw new Error("Incomplete Reviewer accepted-batch identity");
  if (hasBatch) {
    ...
    if (!exactKeys(construction, ["recipe", "bundle"]) || construction.recipe !== "reviewer-common-bundle-v1" || !isRecord(construction.bundle) || !verifyBundleIdentity(construction.bundle as any) ||
        ...
        Object.values(target.refs).some((ref) => !isRecord(ref) || !exactKeys(ref, ["objectId", "peeledCommitId"]) || typeof ref.objectId !== "string" || typeof ref.peeledCommitId !== "string")) throw new Error("Invalid Reviewer construction or target identity");
  }
  ...
  if (output.status === "completed" && (!hasBatch || outcomeCount === 0 || Object.values(output.outcomes).some((item) => (item as RuntimeReviewerOutcome).status !== "successful"))) throw new Error("Completed Reviewer receipt requires a successful accepted batch");
  if (!hasBatch && outcomeCount !== 0) throw new Error("Pre-acceptance Reviewer refusal cannot contain outcomes");
