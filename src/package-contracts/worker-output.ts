...
export type ClassRepairException = { where: string; reason: string };
export type ClassRepaired = {
  name: string;
  searchScope: string;
  exceptions: ClassRepairException[];
};
...
export type FixerOutput = CoderOutput & { classesRepaired?: ClassRepaired[] };
...
function validClassesRepaired(value: unknown): value is ClassRepaired[] {
  ...
  return value.every((entry) => {
    if (!isRecord(entry) || !hasExactKeys(entry, ["name", "searchScope", "exceptions"]) ||
      ...
      !Array.isArray(entry.exceptions) || names.has(entry.name)) return false;
...
export function validateAcceptedWorkerDetails(
  ...
  if (!isRecord(output)) throw new Error(`${roleLabel} output must be an object`);
  ...
  return {
    ...
    ...(hasClasses ? { classesRepaired: (output.classesRepaired as ClassRepaired[]).map((entry) => ({
