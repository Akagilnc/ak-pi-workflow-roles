...
export const OMIT_NAVIGATOR_FLAG = Object.freeze({
  name: "ak-omit-navigator",
  definition: Object.freeze({
    description: "Omit automatic navigator attendance (#840)",
    type: "boolean" as const,
  }),
} as const);
...
export function createRoleRuntimeExtension(
  ...
  return (envelopeHost) => {
    ...
    // Station child flags (#840): omit navigator attendance.
    roleHost.registerFlag(STATION_CHILD_FLAG.name, STATION_CHILD_FLAG.definition);
    roleHost.registerFlag(OMIT_NAVIGATOR_FLAG.name, OMIT_NAVIGATOR_FLAG.definition);
    ...
    roleHost.on("session_start", async (event, ctx) => {
      ...
      try {
        ...
        const isStationChild =
          ...
          roleHost.getFlag(OMIT_NAVIGATOR_FLAG.name) === true;
        if (
