    ...
    hostActions !== undefined && roleHost.requireGatekeeperPass !== undefined
      ? async ({ toolCallId, parameters, signal, ctx }) => {
          ...
          if (status === undefined || !COUNTERSIGN_QUEUE_STATUSES.has(status)) {
            // Parent status unreadable → back to countersign; do not summon notary.
            const nonPass = {
              status: "bounce" as const,
              officer: "notary" as const,
              receipt: parameters === undefined ? { missing: "arguments" as const } : parameters,
            };
            // Plain re-ask is the model-visible tool result; receipt stays on details.
            hostActions.bindSubmissionNonPass(toolCallId, nonPass);
            throw new GatekeeperDecisionError(nonPass, COUNTERSIGN_STATUS_REASK);
          }
          if (status === "escalate") {
