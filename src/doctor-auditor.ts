...
import type { DoctorEvidenceIndexV1, DoctorOutput } from "./doctor-contracts.ts";

export const DOCTOR_AUDIT_TOOL_NAME = "ak_doctor_audit_decision";
export type DoctorAuditInput = { soul: string; index: DoctorEvidenceIndexV1; readRecord: Array<{ evidenceId: string; fullyRead: boolean }>; output: DoctorOutput };
...
export function createPiDoctorAuditor(runCompletion?: ComplianceCompletion) {
  return (input: DoctorAuditInput, options: { context: ExtensionContext; signal?: AbortSignal }): Promise<ComplianceDecision> => runComplianceAudit({
    tool,
    systemPrompt: [
    ...
    serializedInput: ["<doctor_soul>", input.soul, "</doctor_soul>", "<frozen_evidence_index>", JSON.stringify(input.index), "</frozen_evidence_index>", "<read_record>", JSON.stringify(input.readRecord), "</read_record>", "<proposed_doctor_output>", JSON.stringify(input.output), "</proposed_doctor_output>"].join("\n"),
