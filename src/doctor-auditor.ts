...
export function createPiDoctorAuditor(runCompletion?: ComplianceCompletion) {
  return (input: DoctorAuditInput, options: { context: ExtensionContext; signal?: AbortSignal }): Promise<ComplianceDecision> => runComplianceAudit({
    ...
    serializedInput: ["<doctor_soul>", input.soul, "</doctor_soul>", "<frozen_evidence_index>", JSON.stringify(input.patient), "</frozen_evidence_index>", "<read_record>", JSON.stringify(input.readRecord), "</read_record>", "<proposed_doctor_output>", JSON.stringify(input.output), "</proposed_doctor_output>"].join("\n"),
