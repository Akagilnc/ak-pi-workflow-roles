/**
 * Analyst model-group mode kernel (PRD #298 output ④ / ticket #331).
 *
...
export type AnalystModelGroupsPage = {
  readonly kind: "analyst-model-groups";
  readonly mode: "model-groups";
  /** Caller-typed scope (physical identities), stable-sorted. */
  readonly projectRoots: readonly string[];
  readonly groups: readonly AnalystModelGroupRow[];
 ...
 */
export function analystModelGroupKey(models: readonly string[]): string | undefined {
 ...
 */
function modelIdentityAbsentEntry(run: AnalystReadableRunFacts): AnalystUnreadableRun {
  return {
    runId: run.runId,
    book: run.book,
 ...
 */
export function buildAnalystModelGroupsPage(input: {
  readonly projectRoots: readonly string[];
  ...
  const groupedRuns: { run: AnalystReadableRunFacts; rawGroupKey: string }[] = [];
  const modelAbsent: AnalystUnreadableRun[] = [];
  for (const run of input.runs) {
    const rawGroupKey = analystModelGroupKey(run.models);
  ...
  for (const { run, rawGroupKey } of groupedRuns) {
    ...
    if (leg === undefined) {
      ...
      throw new Error(
        `analyst model-groups: missing acceptance projection for run ${run.runId}`,
      );
    }
  ...
  const groups: AnalystModelGroupRow[] = rawKeys.map((rawGroupKey) => {
