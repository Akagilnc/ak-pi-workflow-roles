...
export type AuditorRoleOptions = { systemPrompt: string; serializedInput: string; tool: AuditorDecisionTool; roleLabel: string; context: ExtensionContext; signal?: AbortSignal; runCompletion?: AuditorCompletion; retainResponse?(response: AssistantMessage): void };
...
export async function executeAuditorChild(options: AuditorRoleOptions): Promise<{ decision: unknown; response: AssistantMessage }> {
  ...
  const createRetriedStream = (simple: boolean, model: Model<Api>, context: Context, request?: ProviderStreamOptions): ReturnType<Provider["stream"]> => {
    ...
    void (async () => {
      for (let attempt = 0; ; attempt += 1) {
        ...
        try {
          ...
          if (isStreamIdleTimeoutError(failure) && attempt < DEFAULT_COMPLIANCE_IDLE_MAX_RETRIES && options.signal?.aborted !== true) continue;
  ...
  const provider: Provider = {
    ...
    stream(model, context, request) { return createRetriedStream(false, model, context, request as ProviderStreamOptions | undefined); },
    streamSimple(model, context, request) { return createRetriedStream(true, model, context, request as ProviderStreamOptions | undefined); },
  };
  ...
  try {
    ...
    const unsubscribe = session.subscribe((event) => {
      if (event.type === "message_end" && event.message.role === "assistant" && boundaryResponse === undefined) {
        ...
        if (turns >= AUDITOR_TURN_LIMIT && boundaryResponse === undefined) boundaryResponse = event.message;
      }
    ...
    try {
      ...
      const assistants = [...session.messages].reverse().filter((message): message is AssistantMessage => message.role === "assistant");
      ...
      if (response === undefined) throw new Error(`${options.roleLabel} exited without a terminal response`);
      try {
