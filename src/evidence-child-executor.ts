...
function classifiedError(error: unknown, evidenceChildFailure: "provider" | "child"): ClassifiedReviewerError {
  ...
  const classification = "evidenceChildFailure" in wrapped ? (wrapped as ClassifiedReviewerError).evidenceChildFailure : evidenceChildFailure;
...
export async function executeEvidenceChild(
  ...
  const childConfigDir = await mkdtemp(
    join(options.credentialScratchParent ?? tmpdir(), "ak-evidence-child-"),
  );
