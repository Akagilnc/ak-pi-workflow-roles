...
export type SessionOwner = Identity & { path: string };

const canonicalRowId =
...
function assertRealAncestors(root: string, path: string): void {
  let current = root;
  try {
...
export function createSessionLeaf(config: RecorderConfig): SessionOwner {
...
export function readSession(
  ...
  owner: SessionOwner,
  onRow: (row: unknown, index: number) => void = () => {},
): SessionSnapshot {
  const directoryStats = modified(() => lstatSync(owner.path));
  if (
    inode(directoryStats) !== inode(owner) ||
  ...
  for (const name of names) {
    ...
    if (
      ...
      mainNames.push(name);
    else throw new RecorderError("session-ambiguous");
  }
  if (!mainNames.length) throw new RecorderError("session-missing");
  if (mainNames.length !== 1) throw new RecorderError("session-ambiguous");
  ...
  if (noFollow === undefined) throw new RecorderError("session-ambiguous");
  ...
  try {
    ...
    if (!before.isFile()) throw new RecorderError("session-ambiguous");
    ...
    const verify = () => {
      ...
      if (
        inode(currentFile) !== inode(before) ||
