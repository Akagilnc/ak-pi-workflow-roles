...
function observedCommits(row: Record<string, unknown>): string[] {
  ...
  for (const match of text.matchAll(/(?:HEAD|commit(?:Sha|SHA)?|sha)[^0-9a-f]{0,30}([0-9a-f]{7,40})/gi)) observations.push(match[1]!);
  return observations;
}
