...
function observedCommits(row: Record<string, unknown>): string[] { const observations: string[] = []; for (const match of JSON.stringify(row).matchAll(/(?:HEAD|commit(?:Sha|SHA)?|sha)[^0-9a-f]{0,30}([0-9a-f]{7,40})/gi)) observations.push(match[1]!); return observations; }
...
function deriveSession(content: string, id: string): SessionDerivation {
  ...
  for (const row of rows) {
    for (const commit of observedCommits(row)) if (commit !== observedCommit) { commits.push({ source: id, commit }); observedCommit = commit; }
