...
type MetricAccumulator = { count: number; sources: string[] };
function accumulate(metric: MetricAccumulator, value: number, source: string) { metric.count += value; if (value) metric.sources.push(source); }
...
export async function loadDoctorCase(runsPath: string): Promise<DoctorCase> {
  ...
  const turns: MetricAccumulator = { count: 0, sources: [] }, calls: MetricAccumulator = { count: 0, sources: [] }, tokens: MetricAccumulator = { count: 0, sources: [] };
  for (const path of await discoverCaseFiles(root)) {
    ...
    const result = deriveSession(content, id); sessions.push(result.session); statuses.push(...result.statuses); commits.push(...result.commits); accumulate(turns, result.turns, id); accumulate(calls, result.calls, id); accumulate(tokens, result.tokens, id);
  }
