...
import { verifyBundleIdentity, type PinnedMechanicalBundleV1 } from "./reviewer-construction.ts";
...
export async function materializeMechanicalBundle(workspace: string, leg: "standards" | "spec", bundle: PinnedMechanicalBundleV1): Promise<MaterializedBundleEvidenceV1> {
  if (!verifyBundleIdentity(bundle) || bundle.entries.some(({ bytes }) => typeof bytes !== "string")) throw new Error("Mechanical bundle digest or manifest mismatch");
  const root = await realpath(workspace);
  const destinations = new Set<string>();
  const staged: Array<{ temporary: string; destination: string }> = [];
  try {
    for (const item of bundle.entries) {
      if (isAbsolute(item.relativeClonePath) || item.relativeClonePath.includes("\\") || item.relativeClonePath.split("/").some((part) => !part || part === "." || part === "..")) throw new Error("Mechanical bundle path is not confined");
      const destination = resolve(root, item.relativeClonePath);
      if (!confined(root, destination) || destinations.has(destination.normalize("NFC"))) throw new Error("Mechanical bundle path collision or escape");
      destinations.add(destination.normalize("NFC"));
      const parent = dirname(destination);
      await mkdir(parent, { recursive: true });
      let cursor = parent;
      while (cursor !== root) {
        if ((await lstat(cursor)).isSymbolicLink()) throw new Error("Mechanical bundle parent is a symlink");
    ...
    for (const item of bundle.entries) {
      ...
      if (!stat.isFile() || stat.isSymbolicLink()) throw new Error("Mechanical bundle readback is not a regular file");
