...
export function createPiFixerAuditor(runCompletion?: ComplianceCompletion): (input: FixerAuditInput, options: { context: ExtensionContext; signal?: AbortSignal }) => Promise<ComplianceDecision> {
  return async (input, options) => runComplianceAudit({
    ...
    systemPrompt: [
      ...
      "Check only demonstrated Soul, packet, and invocation-record compliance, including honest coverage and lawful dispositions.",
      ...
      "Treat prerequisite_unmet prospectively and within the assigned work: at disposition time, a specific required predecessor artifact, decision, or state must still be absent, and that absence must make the remaining assignment presently unlawful to execute.",
    ...
    serializedInput: [
      ...
      "<fix_packet>", input.packet, "</fix_packet>",
