...
export type DoctorFinding = { targetKey: string; targetKind: DoctorTargetKind; evidenceIds: string[]; disposition: "keep" | "thin" | "delete"; recommendation: string };
...
export function validateDoctorOutput(value: unknown, patient: DoctorCase, store: DoctorEvidenceStore): DoctorOutput {
  const output = validateRecordedDoctorOutput(value); if (output.status === "refused") return output;
  ...
  for (const finding of output.findings) for (const id of finding.evidenceIds) if (!store.entries.has(id) || !store.hasRead(id)) throw new Error(`finding must cite admitted/read evidence: ${id}`);
  return output;
}
