...
/** Production role command → its public argv parser (the real attach contract). */
const PUBLIC_ROLE_ARGV_PARSER = {
  judge: parseJudgeArgv,
  coder: parseCoderArgv,
  fixer: parseFixerArgv,
  collector: parseCollectorArgv,
  doctor: parseDoctorArgv,
  merger: parseMergerArgv,
  reviewer: parseReviewerArgv,
} as const;

export type PublicRoleArgvCommand = keyof typeof PUBLIC_ROLE_ARGV_PARSER;

function isPublicRoleArgvCommand(command: string): command is PublicRoleArgvCommand {
  return Object.prototype.hasOwnProperty.call(PUBLIC_ROLE_ARGV_PARSER, command);
}

/**
 * Whether a public role's real `parse*Argv` contract accepts `--attach`.
 * Derived by exercising the production parser — not a parallel capability set.
 */
export function publicRoleAcceptsAttach(command: string): boolean {
  if (!isPublicRoleArgvCommand(command)) return false;
  const parse = PUBLIC_ROLE_ARGV_PARSER[command];
  try {
    parse(["--attach", "/.ak-roles-attach-probe"]);
    return true;
  } catch (error) {
    if (
    ...
    // Flag was accepted; failure is about other required shape (e.g. --issue).
    return true;
  }
}
