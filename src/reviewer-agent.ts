...
export type ReviewerFailedLegRunResult = Readonly<{ status: "failed"; failure: ReviewerFailureClassification; target: ReviewerTargetSnapshot; prompt: ReviewerPromptIdentity; workspaceDisposition: ReviewerWorkspaceDisposition }>;
...
function failed(error: unknown, target: ReviewerTargetSnapshot, prompt: ReviewerPromptIdentity, signal?: AbortSignal, retained?: string): ReviewerFailedLegRunResult { const attached = typeof error === "object" && error !== null ? error as { targetSnapshot?: ReviewerTargetSnapshot; workspaceDisposition?: ReviewerWorkspaceDisposition } : {}; return Object.freeze({ status: "failed", failure: classify(error, signal), target: attached.targetSnapshot ?? target, prompt, workspaceDisposition: retained === undefined ? attached.workspaceDisposition ?? "not-created" : { retained } }); }
  ...
  return {
    async run(dispatch, options) {
      ...
      const settled = await Promise.allSettled(batch.workspaces.map(async workspace => {
        ...
        catch (error) { return [leg.axis, failed(error, batch.target, leg.prompt, options.signal, workspace.path)] as const; }
      }));
