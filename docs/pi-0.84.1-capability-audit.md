...
| Codex fast file switch | **keep** | 0.84.1 accepts an explicit `serviceTier`, but has no equivalent file switch. The single 0.84.1 patch remains the sole source. Upgrade maintenance simply regenerates and applies that one patch, then performs one fast-on wire smoke through the existing provider seam to observe `service_tier=priority`. |
