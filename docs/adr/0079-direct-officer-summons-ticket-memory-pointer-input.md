...
**三次修订（2026-09-07，owner / #747）：** 内闸官（符宝郎／察院／审刑院）按票号找该席最新 run 会跨父腿 resume 终局旧记忆。owner 点头改法：查找键从票号换成本次父 run 路径；三席一并（L119882）。`ticket-seat-memory-countersign-principal`、`explicit-fresh-summons` 不动。不加旗、不加轮次、不加字段、不加重读票面行。
