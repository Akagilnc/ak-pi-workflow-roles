...
**本条以上正文写的是软规则**——它写在 md 里，供派单与席位配置据以要求外包；**实际不用它拦已经写好的工作**。

具体地：席位挂了 `engine` 而某条 run 零次调用该引擎，**不构成交卷不合法**。任何衙门（判官、票庭、闸官）**不得据本条**撤销、作废、重跑该 run，或拒绝结算其产出。runner 的义务只到派单——在派单文里写明外包与型号；腿不照办，不再追究。
