...
- **不 supersede [ADR 0066](0066-worker-typed-submission-gates.md)**：修内司合规审计腿退役维持、将作监合规腿仍不加。给事中不是合规腿复活：法不同——0066 退役的是与下游四层兜底重叠的**合规**审计（观察窗内零真捕获）；给事中执掌的复杂度/测试质量两轴现无人执掌（陛下观察：「御史台现在检查出来的复杂度和测试简化/必要性 感觉几乎没有」）。且是否默认挂在 worker run 上未定，归编排后议。
