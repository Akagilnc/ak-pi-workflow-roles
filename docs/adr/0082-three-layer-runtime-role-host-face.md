...
Reviewer 默认在一次调用内并行执行 completeness 与 correctness 两轴；`--lens completeness|correctness` 仅作为显式单轴覆盖。各轴复用 canonical `ak-cross-m-review` 的既有单轴路径；具体并行上下文与隔离由有效 host×engine 的原生能力承载，能力不足时具名失败，不得串行或静默换轴。
