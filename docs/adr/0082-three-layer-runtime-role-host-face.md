...
公开 Reviewer 父调用默认经共享执行接缝并行派发 completeness 与 correctness 两条独立单轴 Reviewer 子调用，并在一个父终局中汇合两份原始 Terminal；`--lens completeness|correctness` 仅作为显式单轴覆盖。各子调用复用 canonical `ak-cross-m-review` 的既有单轴路径，分别拥有 run、session 与 typed 出口；父层不得串行、吞掉或改标任一子终局。
