...
被修订的一句：0033 中「`bounce` 要求 `status: "bounce"` 与非空 `violations`」的 **runtime 强制**部分（#754 前结论词曾作 `revise`，现役统一为 `bounce`）。修订后：该要求由四席审刑院 soul 承担，runtime 不再因 `violations` 为空而拒收。0033 的其余部分（`pass` 只要求 `status`，其他内容一概不管）不变。

为何不能默删：它防的是「打回却不说理由」。配合 [ADR 0007](0007-no-audit-retry-brake.md)「审计重交不设重试帽」，一个空 `violations` 的 bounce 即无理由死循环——真实可复现的失败，护栏三问全过。删除它必须有承接者（CONTEXT.md「承接者判据」）。
...
| `souls/fixer-auditor.md` | 仅「明确违反 Soul 则 `bounce` 并逐条指出」，缺后半句 | 补齐后半句 |
...
1. **`bounce` 必须逐条给出违反条目与原因**——由各 auditor soul 承担（`judge-auditor` / `reviewer-auditor` / `doctor-auditor`；以及磁盘留存的 dormant `fixer-auditor`）。此条不是 audit-law 的无证连续打回规则。
