...
**例外：内审衙门。** 立本 ADR 时尚无此概念。内审衙门与被审席之间是包内既定关联，不由调用者组合——现行实例：交卷闸按受审物直接传召台院／符宝郎，审刑院只挂大理寺（[ADR 0072](0072-menxia-pre-pr-submission-hooks.md) 定挂钩映射「将作监/修内司 completed → 质检席；大理寺拟判入卷后 → 符宝郎；审刑院只挂大理寺、零改动」，[ADR 0079](0079-direct-officer-summons-ticket-memory-pointer-input.md) 后改为直接传召并载同父 run 内 resume；[ADR 0062](0062-auditor-is-an-independent-substantive-role.md) 载审刑院为独立实质审计角色，不规定拓扑）。本条按「内审衙门」整类立，不以上列实例为穷举。
