...
**具名例外：Reviewer 默认双轴父调用。** 公开 Reviewer 父调用省略 `--lens` 时，由外层共享执行接缝机械并行派发 completeness 与 correctness 两条单轴 Reviewer 子调用并汇合终局；显式 `--lens` 不派双轴。本例外不改变 Reviewer 的寺监级身份，也不建立通用编排器。
...
> 「当初设计的时候我的原意大概是这样
> 角色之间没有强关联（内审衙门除外，那会还没有这个概念）。不再试图造一个编排器（编排器v1v2和目前这个v3离石自己去wiki读）。专心做好每一个角色。角色之前没有强硬的关联。调用者想怎么调用都行，比如想喊给事中/大理寺来审一下某个想法或者票面都可以。而不是像v1v2那样死板和沉重」
