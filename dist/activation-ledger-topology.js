 ...
 */
export function resolveActivationLedgerHome(home = () => process.env.HOME ?? homedir()) {
    const processHome = home();
    if (typeof processHome !== "string" || processHome.length === 0 || !isAbsolute(processHome)) {
        throw new ActivationLedgerError(`activation ledger process home must be absolute, got ${JSON.stringify(processHome)}`);
    }
    return resolve(processHome, ".ak-roles");
}
/** Enumerable book directory for one basename key. */
export function activationBookDirectory(ledgerHome, bookKey) {
    return join(ledgerHome, "books", bookKey);
}
/** Append-only waiting ledger path for one book. */
export function activationWaitingLedgerPath(ledgerHome, bookKey) {
...
export function pathContainedIn(root, candidate) {
 ...
 */
export function physicalPathIdentity(path) {
...
export function physicallyContainedIn(root, candidate) {
...
function assertPhysicalLedgerRoot(absoluteRoot) {
    ...
    try {
        ...
        if (errnoCode(error) !== "ENOENT") {
            throw new ActivationLedgerError(`activation ledger failed to stat home (${absoluteRoot}): ${errorText(error)}`, { cause: error });
        }
    }
    if (st === undefined) {
        try {
            ...
            if (errnoCode(error) !== "EEXIST") {
                throw new ActivationLedgerError(`activation ledger failed to create home (${absoluteRoot}): ${errorText(error)}`, { cause: error });
            }
        }
        try {
            st = lstatSync(absoluteRoot);
    ...
    if (st.isSymbolicLink()) {
        throw new ActivationLedgerError(`activation ledger home is a symbolic link: ${absoluteRoot}`);
    }
    if (!st.isDirectory()) {
 ...
 */
export function ensureRealDirectoryTree(root, targetDir) {
    if (!isAbsolute(root)) {
    ...
    if (absoluteTarget !== absoluteRoot && !pathContainedIn(absoluteRoot, absoluteTarget)) {
        throw new ActivationLedgerError(`activation ledger path escapes ledger home (${absoluteRoot}): ${absoluteTarget}`);
    }
    ...
    try {
        ...
        throw new ActivationLedgerError(`activation ledger home is not resolvable (${absoluteRoot}): ${errorText(error)}`, { cause: error });
    }
    ...
    for (const part of rel.split(sep)) {
        ...
        try {
            ...
            if (errnoCode(error) !== "ENOENT") {
                throw new ActivationLedgerError(`activation ledger failed to stat path component (${lexicalCursor}): ${errorText(error)}`, { cause: error });
            }
            try {
                ...
                if (errnoCode(mkdirError) !== "EEXIST") {
                    throw new ActivationLedgerError(`activation ledger failed to create directory (${lexicalCursor}): ${errorText(mkdirError)}`, { cause: mkdirError });
                }
            }
            try {
                st = lstatSync(lexicalCursor);
                ...
                throw new ActivationLedgerError(`activation ledger failed to stat path component (${lexicalCursor}): ${errorText(statError)}`, { cause: statError });
            }
        }
        ...
        if (st.isSymbolicLink()) {
            throw new ActivationLedgerError(`activation ledger path component is a symbolic link: ${lexicalCursor}`);
        }
        if (!st.isDirectory()) {
        ...
        try {
            ...
            throw new ActivationLedgerError(`activation ledger path component is not resolvable (${lexicalCursor}): ${errorText(error)}`, { cause: error });
        }
        if (realCursor !== realRoot && !pathContainedIn(realRoot, realCursor)) {
            throw new ActivationLedgerError(`activation ledger path component escapes ledger home (${lexicalCursor} -> ${realCursor})`);
        }
    }
    try {
        ...
        throw new ActivationLedgerError(`activation ledger directory is not resolvable (${absoluteTarget}): ${errorText(error)}`, { cause: error });
    }
}
/**
 ...
 */
export function assertLedgerFileInsideHome(ledgerPath, ledgerHome) {
    ...
    try {
        if (!lstatSync(resolvedLedger).isSymbolicLink())
            return;
        throw new ActivationLedgerError(`activation ledger file is a symbolic link: ${resolvedLedger}`);
    }
    catch (error) {
        if (errnoCode(error) !== "ENOENT") {
            if (error instanceof ActivationLedgerError)
                throw error;
            throw new ActivationLedgerError(`activation ledger failed to stat ledger file (${resolvedLedger}): ${errorText(error)}`, { cause: error });
        }
    }
}
