...
init_activation_ledger_git();
init_activation_ledger_topology();
import { dirname as dirname11, isAbsolute as isAbsolute4, join as join16, resolve as resolve12, basename as basename5 } from "node:path";
import { lstat as lstat2, realpath as realpath4 } from "node:fs/promises";

// src/public-cli/run-lifecycle.ts
init_activation_ledger_topology();
import { chmod, open, readdir as readdir2, readFile as readFile10, unlink as unlink2, writeFile as writeFile6 } from "node:fs/promises";
...
import { basename as basename4, isAbsolute as isAbsolute3, join as join14, resolve as resolve11, sep as sep3 } from "node:path";
...
async function loadAdmittedJudgeRequest(runDirectory) {
  try {
    const raw = JSON.parse(
      await readFile9(join14(runDirectory, "admitted-request.json"), "utf8")
    );
...
async function readRoleRunStateDisk(runDirectory) {
  let raw;
  try {
    raw = JSON.parse(await readFile10(join15(runDirectory, RUN_STATE_FILE), "utf8"));
  } catch {
    return void 0;
  }
...
async function loadNavigatorWorkContext(options) {
  ...
  const input = reference === void 0 || options.role === "doctor" || options.role === "notary" ? void 0 : await readFile11(reference, "utf8");
...
function resolvePackagedMethodSkillRoot(packageRoot, name) {
  return join17(packageRoot, packagedMethodSkillRelativeDirectory(name));
}
function resolvePackagedMethodSkillPath(packageRoot, name) {
  return join17(resolvePackagedMethodSkillRoot(packageRoot, name), "SKILL.md");
}
...
async function loadPackagedMethodSkillMaterial(packageRoot, name) {
  ...
  const skillPathConfigured = join17(rootDirectory, "SKILL.md");
  const provenancePath = join17(rootDirectory, "provenance.json");
  let provenanceRaw;
  try {
  ...
  for (const [rel, expected] of Object.entries(provenance.files)) {
    const absolute = join17(rootDirectory, rel);
    let bytes;
    try {
  ...
  try {
    ...
    raw = await readFile12(skillPath, "utf8");
  } catch (error) {
...
async function loadPackagedCanonicalSkillBinding(packageRoot, name) {
  ...
  const snapshot = Object.freeze({
    ...
    baseDir: dirname12(material.skillPath),
    body: material.body,
...
async function prepareSnapshot(accepted, signal, dependencies) {
  ...
  try {
    ...
    mirrorRoot = await mkdtemp4(join18(tmpdir4(), "ak-reviewer-snapshot-"));
    const mirrorPath = join18(mirrorRoot, "repository.git");
...
async function prepareClone(snapshot, signal, dependencies) {
  ...
  try {
    ...
    workspace = await mkdtemp4(join18(tmpdir4(), "ak-reviewer-leg-"));
...
import { basename as basename6, dirname as dirname13, join as join20 } from "node:path";
...
import { dirname as dirname14, isAbsolute as isAbsolute6, resolve as resolve15 } from "node:path";
...
function appendActivationLedgerLine(ledgerPath, line2, options) {
  ...
  const parent = dirname14(resolvedLedger);
...
var gatekeeperDecisionSchema = openToolObject(Type22.Object({
  status: Type22.Unknown({ description: "dispatch | pass \u2014 \u5F62\u72B6\u6307\u5F15\uFF0C\u975E schema \u95F8" }),
  officer: Type22.Unknown({ description: "status \u4E3A dispatch \u65F6\u4E3A inspector | notary" }),
  findings: Type22.Unknown({ description: "status \u4E3A pass \u65F6\u53EF\u9009 string[] findings" })
}));
...
import { mkdir as mkdir5, readFile as readFile13, writeFile as writeFile8 } from "node:fs/promises";
import { createServer } from "node:net";
import { appendFileSync as appendFileSync2 } from "node:fs";
import { basename as basename8, dirname as dirname16, join as join23 } from "node:path";
...
async function installGrokPreToolUseDeny(controlledHome) {
  const hooksDir = join21(controlledHome, "hooks");
  await mkdir3(hooksDir, { recursive: true });
  const scriptName = "ak-bash-seatbelt.mjs";
  const scriptPath = join21(hooksDir, scriptName);
  await writeFile7(scriptPath, renderGrokBashSeatbeltHookScript(), { mode: 493 });
  await writeFile7(join21(hooksDir, "ak-bash-seatbelt.json"), `${JSON.stringify({
    hooks: {
      PreToolUse: [
        {
...
async function resolveHeadTreePath(topLevel, relativePath) {
  ...
  const hits = listing.split("\n").map((name) => name.trim()).filter((name) => name !== "" && basename7(name).toLowerCase() === needle).map((name) => parent === "." ? basename7(name) : join22(parent, basename7(name)));
...
async function isHeadMatchedProjectInstruction(repositoryCwd, absolutePath) {
  ...
  const candidate = join22(parent, leaf);
  ...
  const headFile = join22(topLevel, headRel);
...
function buildGrokSkillExpansion(methodSkills, prompt) {
  ...
  return Object.freeze({
    ...
    content: `References are relative to ${dirname16(method.path)}.
...
async function prepareGrokRoleEnvelope(options) {
  ...
  for (const method of request.methods) {
    ...
    const name = basename8(dirname16(method.path));
    const raw = await readFile13(method.path, "utf8");
    methodSkills.set(name, { path: method.path, body: stripSkillFrontmatter(raw).trim() });
  }
  let sessionFile = join23(request.runDirectory, "session", "session.jsonl");
  await mkdir5(dirname16(sessionFile), { recursive: true });
  try {
    await writeFile8(
      sessionFile,
      `${JSON.stringify({
...
`,
  ...
  {
    const raw = await readFile13(sessionFile, "utf8");
    for (const line2 of raw.split("\n")) {
      if (line2.trim() === "") continue;
  ...
  const context = {
    ...
    sessionManager: {
      ...
      getSessionDir: () => join23(request.runDirectory, "session"),
...
`);
  ...
  const methodPrompt = (await Promise.all(request.methods.map(({ path }) => readFile13(path, "utf8")))).join("\n\n");
...
import { mkdir as mkdir6, readFile as readFile14, rename, writeFile as writeFile9 } from "node:fs/promises";
import { dirname as dirname17, join as join24 } from "node:path";
function createGrokSessionIdentityAuthority(authority) {
  const bindingPath = (principal) => join24(authority.decode(principal).sessionDirectory, "grok-acp-session.json");
  return {
    async load(principal) {
      try {
        const value = JSON.parse(await readFile14(bindingPath(principal), "utf8"));
    ...
    async bind(principal, sessionId) {
      ...
      await mkdir6(dirname17(target), { recursive: true });
