...
import { isReviewerPromptIdentity, reviewerPromptIdentity } from "./reviewer-prompt-identity.js";
    ...
    return await new Promise((resolve, reject) => {
        ...
        child.on("close", (code) => {
            ...
            if ((options.allowedCodes ?? [0]).includes(actualCode)) {
                ...
                reject(new Error(`${command} ${args.join(" ")} failed (${actualCode}): ${stderr.trim() || stdout.trim()}`));
            }
        });
    });
}
async function git(cwd, args, signal, allowedCodes) {
    return runCommand("git", ["-C", cwd, ...args], {
        ...(signal === undefined ? {} : { signal }),
        ...(allowedCodes === undefined ? {} : { allowedCodes }),
    });
}
async function readRefs(cwd, signal) {
    const result = await git(cwd, reviewerRefSnapshotArgs(), signal);
...
async function verifySnapshot(cwd, snapshot, signal) {
    ...
    if (objectFormat !== snapshot.objectFormat)
...
async function prepareSnapshot(accepted, signal, dependencies = {}) {
    ...
    try {
        ...
        const targetHead = (await git(repositoryRoot, ["rev-parse", "HEAD^{commit}"], signal)).stdout.trim();
        ...
        await runCommand("git", ["clone", "--mirror", "--no-hardlinks", repositoryRoot, mirrorPath], signal === undefined ? {} : { signal });
        const targetPresent = await git(mirrorPath, ["cat-file", "-e", `${targetHead}^{commit}`], signal, [0, 1, 128]);
        ...
        const preservedObjects = Object.values(refs).flatMap((entry) => entry.peeledCommitId === null
            ? [entry.objectId]
...
async function prepareWorkspace(snapshot, signal, dependencies = {}) {
    let workspace;
    try {
...
async function runChild(workspace, leg, context, signal, fault) {
...
function failedLegEvidence(error, fallbackTarget, prompt, signal, retainedWorkspace) {
    const attached = typeof error === "object" && error !== null ? error : undefined;
        ...
        async run(dispatch, options) {
            ...
            const settled = await Promise.allSettled(prepared.map(async ({ leg, workspace, evidence }) => {
                try {
                    ...
                    return [leg.axis, Object.freeze({ status: "successful", report: child.report, usage: child.usage, target, prompt: child.prompt, workspaceDisposition: "deleted", runtimeConstructionEvidence: evidence })];
                }
                catch (error) {
                    return [leg.axis, failedLegEvidence(error, target, leg.prompt, options.signal, workspace)];
                }
            }));
            const pairs = settled.map((item) => item.status === "fulfilled" ? item.value : (() => { throw item.reason; })());
            const outcome = Object.freeze({ identity: dispatch.identity, target, legs: Object.freeze(Object.fromEntries(pairs)) });
            if (Object.values(outcome.legs).some((leg) => leg?.status === "failed"))
                throw new ReviewerDispatchExecutionError(outcome);
            return outcome;
        },
        async shutdown() {
            if (snapshotPromise === undefined || snapshotDeleted)
                return;
