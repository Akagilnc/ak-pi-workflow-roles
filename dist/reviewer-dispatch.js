...
export const REVIEWER_PREFLIGHT_VIOLATIONS = ["base-invalid", "range-invalid", "prompt-identity-invalid", "prompt-identity-mismatch", "target-drift"];
...
export function toReviewerExecution(dispatch) { return Object.freeze({ identity: dispatch.identity, recipe: dispatch.recipe, targetSnapshot: immutableReviewerPin(dispatch.targetSnapshot), bundle: dispatch.bundle, legs: Object.freeze(dispatch.legs.map(l => Object.freeze({ axis: l.axis, prompt: Object.freeze({ ...l.prompt }) }))) }); }
const preflight = (error) => { if (error instanceof ReviewerPreflightError)
    return error; if (error instanceof ReviewerConstructionError)
    return new ReviewerPreflightError(error.code, error.diagnostic); if (error instanceof ReviewerCorrectablePreflightError)
    return new ReviewerPreflightError(error.code, error.diagnostic); return undefined; };
export function createReviewerDispatcher(d) {
    const task = Uint8Array.from(d.task), target = immutableReviewerPin(d.reader.pin);
    let started = false;
    return Object.freeze({ async dispatch(baseRevision, invocation) { if (started)
            throw new Error("Reviewer fixed dispatch can start exactly once"); let taskText; try {
        ...
        } ; const identity = sha256Hex(JSON.stringify({ task: sha256Hex(task), baseRevision, target: target.targetHead })); let dispatch; try {
            const base = await d.reader.resolve(baseRevision);
            const range = await d.reader.range(base);
            dispatch = constructReviewerDispatch({ identity, taskText, canonicalSkill: d.canonicalSkill, target, range, ...(d.reviewScopeKeys === undefined ? {} : { reviewScopeKeys: d.reviewScopeKeys }), ...(d.compilePrompt === undefined ? {} : { compilePrompt: d.compilePrompt }) });
        ...
        } started = true; d.decisionEvidence?.(Object.freeze({ disposition: "accepted", identity, dispatch })); const results = await d.run(toReviewerExecution(dispatch), invocation); return Object.freeze({ status: "accepted", dispatch, results }); } });
}
