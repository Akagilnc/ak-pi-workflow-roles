...
export function validateFixerOutput(value, phase) {
    ...
    if (value.status === "unfinished") {
        if (phase === "plan")
            fail("status phase constraint: plan forbids unfinished");
        if (!nonblank(value.remainingScope))
            fail("remainingScope nonblank constraint");
        if (Object.hasOwn(value, "blocker") || Object.hasOwn(value, "commitSha"))
            fail("status unfinished semantic-field combination constraint");
        if (!Object.hasOwn(value, "classResults"))
            return { status: "unfinished", report: value.report, remainingScope: value.remainingScope };
        if (!Array.isArray(value.classResults) || value.classResults.length === 0)
            fail("unfinished classResults nonempty array constraint");
        const names = new Set();
        const classResults = value.classResults.map((item, index) => {
            const path = `classResults[${index}]`;
            if (!record(item) || item.disposition !== "completed")
                fail(`${path} unfinished completed-only constraint`);
            ...
            const exceptions = item.exceptions.map((entry, exceptionIndex) => {
                ...
                if (!record(entry) || !nonblank(entry.where) || !nonblank(entry.reason))
                    fail(`${exceptionPath} nonblank constraint`);
