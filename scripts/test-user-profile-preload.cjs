 ...
 *
 * Before patching, preserve the real passwd home in AK_TEST_REAL_MACHINE_HOME so
 * test-agent-dir-guard can still refuse writes to the operator's ~/.pi/agent
 * (that guard must not follow the temporary profile).
 *
