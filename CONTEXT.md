...
- **Reviewer(御史台)**:围绕一个固定目标形成独立、可追溯代码评审的角色;不修复、不发布、不路由、不作最终裁决。强制装载包内 canonical `ak-cross-m-review` 方法 Skill（中层公共步骤；Pi adapter 以原生 `/skill:ak-cross-m-review` 展开，非 Pi 宿主经 typed methods／systemPrompt 面送达，ADR 0082）；公开 `--lens completeness|correctness` 由调用者选定单 lens，本席自执行；回执只表达 `completed` 或 `refused`，amendments 轴为 completeness/correctness，不表达批准、合并或流转语义。
