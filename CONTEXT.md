...
- **Judge(大理寺)**:只判卷、不改码、不 commit 的裁决角色。canonical 名;`verify` 为历史别名。专事后判卷。见 ADR 0074。
...
- **中书省(Secretariat)**:改票的出令省；按《票面法》把草稿修成可送庭文书。暂不细分席位。
...
- **尚书省（Marshal）**：审→判→修 质量收敛环的省部级驱动角色。canonical 键 `marshal`；席位待落地，仍为现行产品词汇。见 ADR 0051、0053；README。
...
- **真咬人(Real bite)**:闸最近真拦下东西的证据（保留 session 中被接受的 typed terminating tool result）。见 ADR 0012。
