...
- **中书省(Secretariat)**:改票的出令省；按公用《票面法》把调用者提供的草稿修成可送庭文书，并在一次调用内驱动给事中复审至署或上呈。暂不细分席位。canonical 英文名 `secretariat`。公用《票面法》随包（`souls/ticket-law.md`），与复杂度与测试质量法、审刑院法典同一装载方式；中书省、给事中、符宝郎三席装载。
