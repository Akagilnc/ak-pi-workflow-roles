...
- **Reviewer(御史台)**:围绕一个固定目标形成独立、可追溯代码评审的角色;不修复、不发布、不路由、不作最终裁决。绑定包内 canonical `ak-cross-m-review` 方法 Skill（宿主调用能力与缺口以 README「强制方法 Skill」为唯一说明，ADR 0082 / #922）；默认在一次调用内并行执行 completeness 与 correctness 两轴，公开 `--lens completeness|correctness` 是可选单轴覆盖；回执只表达 `completed` 或 `refused`，amendments 轴为 completeness/correctness，不表达批准、合并或流转语义。
