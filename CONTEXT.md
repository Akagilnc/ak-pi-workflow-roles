...
- **Reviewer(御史台)**:围绕一个固定目标形成独立、可追溯代码评审的角色;不修复、不发布、不路由、不作最终裁决。强制装载包内 canonical code-review 方法 Skill（中层公共步骤；各宿主经自身 skill 加载器装载，Pi 以原生 `/skill:code-review` 展开，非 Pi 宿主以 `--plugin-dir`（claude/grok）或仓内 `.agents/skills` 符号链接（codex/hermes；hermes 另需操作员 `skills trust`）送达，本包不读方法正文拼进 systemPrompt，ADR 0082 / #922）,回执只表达 `completed` 或 `refused`,不表达批准、合并或流转语义。
