...
- **Reviewer(御史台)**:围绕一个固定目标形成独立、可追溯代码评审的角色;不修复、不发布、不路由、不作最终裁决。强制装载包内 canonical code-review 方法 Skill（中层公共步骤；Pi 以原生 `--skill` 装载，Claude 以原生 `--plugin-dir` 装载；Codex、Hermes、Grok 的调用面不能把随包 Skill 装入任意消费项目，故报告 unsupported-method，本包不补适配层，ADR 0082 / #922）,回执只表达 `completed` 或 `refused`,不表达批准、合并或流转语义。
