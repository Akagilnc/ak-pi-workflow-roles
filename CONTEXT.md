...
- **未完终态(Unfinished)**:两个 worker 角色 apply 阶段的合法交卷状态,只陈述「本次角色调用未结清」这一事实——不诊断原因,不规定调用者的下一步。规范见 [ADR 0047](docs/adr/0047-unfinished-terminal-state-reports-fact-not-diagnosis.md)。#72 已接受新增,须到 #75/#76 施工才落地。
