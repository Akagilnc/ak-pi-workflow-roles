...
- **殿院(Dian-yuan)**:御史台三院名号指针，对位唐制殿中侍御史的朝会定点纠班序。指向 [#560](https://github.com/Akagilnc/ak-pi-workflow-roles/issues/560) 机械定点测试巡查件（产物 kind `test-report`）；**在建/待落地，非席位**，不占机器键。本词条只给名号，不暗示机制已落地。
- **察院(Chayuan)**:御史台三院名号占位，对位唐制监察御史的巡按州县、广度例行扫查。**仅保留名号，巡按机制未建**；本票（#584）不建任何巡按席位、代码或派发路径。
