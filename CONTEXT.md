...
- **Reviewer(御史台)**:围绕一个固定目标形成独立、可追溯代码评审的角色;不修复、不发布、不路由、不作最终裁决。强制装载包内 canonical code-review 方法 Skill（中层公共步骤；Pi 以原生 `--skill` 装载，Claude 以 `--plugin-dir` 加原生 slash-command 调用，Codex 与 Hermes 经官方项目 Skill 发现路径装载；Grok ACP 无对应装载面时报告缺口，本包不补适配层，ADR 0082 / #922）,回执只表达 `completed` 或 `refused`,不表达批准、合并或流转语义。
