...
- **角色(Role)**:有明确职掌、受门禁约束、以交卷物为法定出口的**车间内**治理单元。角色只管一次调用的内政。按派单权分**寺监级**与**省部级**（法律分类）。实现不限于 LLM。见 ADR 0010、0047、0051。
...
- **交卷工具(Submission tool)**:角色具名的 terminating 工具(`ak_<role>_output`)。**回执(Receipt)** = 其产物,角色劳动成果的法定出口。无回执终局是生命周期 typed 事实,不是回执。见 ADR 0003、0041。
