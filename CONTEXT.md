...
- **门下省(Menxia province)**:质量保证**省部级**席位（形状同尚未落地的尚书省／票面或仍写刑部的 marshal）：将作监、修内司、大理寺交卷时调的是省，不点名司官；**这一次内政之内**按**受审物**派给事中、符宝郎、通进司。将作监/修内司仅 `completed` / `partially_completed` 才调门下省并派给事中（`planned` / `refused` / `unfinished` 不调省）；大理寺拟判 → 符宝郎。省不是纯分类词，也不是外层编排器。给事中与符宝郎仍是独立寺监级角色，自己管自己的交卷。规范见 [ADR 0067](docs/adr/0067-menxia-province-founding-jishizhong-fubaolang.md)；开 PR 前交卷挂钩见 [ADR 0072](docs/adr/0072-menxia-pre-pr-submission-hooks.md)；品级修宪见 #140／#145，尚未落法。
_Avoid_:把「门下省」当作通进司的公开角色名；把省当成没有内政的标签；在大理寺拟判上再派给事中；交卷钩点名给事中或符宝郎。
...
- **刑部（Marshal）**：审→判→修 质量收敛环的省部级驱动角色。调用方递票号与 baseline，刑部驱动御史台取证、大理寺裁决、修内司修理滚到收敛（converged 唯庭可判）或 escalate 上呈，交回 typed 报告；不弹、不判、不修，只让链条转到收敛。canonical 英文名 `marshal`；#140 设计定谳，席位待落地。#145 拟将本席中文名改为**尚书省**，票面可能仍写刑部。
_Avoid_:把省部级内政当成外层编排器；把「刑部」和「尚书省」当成两个席。
