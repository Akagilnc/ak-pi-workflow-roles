...
- **Reviewer(御史台)**:围绕一个固定目标形成独立、可追溯代码评审的寺监级角色;不派 worker、不修复、不发布、不路由、不作最终裁决。绑定包内 canonical `ak-cross-m-review` 方法 Skill（ADR 0082 / #922）；单次角色调用只执行一个冻结 lens。公开 Reviewer 父调用省略 `--lens` 时由外层共享执行接缝并行派发 completeness 与 correctness 两条独立单轴调用并汇合终局，显式 `--lens completeness|correctness` 只跑该轴；回执只表达 `completed` 或 `refused`，amendments 轴为 completeness/correctness，不表达批准、合并或流转语义。
...
_Avoid_:门下省（那是省名）。
- **评审腿(Review leg)**:公开 Reviewer 父调用省略 `--lens` 时，共享执行接缝并行派发的 completeness／correctness 独立单轴 Reviewer child run；Reviewer 角色本体仍不派 worker。Collector 的可选请求不构成评审腿或身份期待。
...
- **角色运行(Role run)**:一次已受理角色调用的持久执行身份，连接其调用请求、执行卷宗与终局结果，并可在用户改选模型后继续同一现场。有模型 run 连接宿主 session；无模型 run 由共享生命周期建立 typed 卷宗主体，不伪造宿主 session。
