...
The capability file is a closed JSON object with exactly `version` (`1`), lowercase `taskSha256`, and unique arrays `tools`, `bashCommands`, and `prerequisiteOperations`. Tool vocabulary is `read|grep|find|ls|bash|write|edit`; prerequisite vocabulary is `preflight.git.pin-target|preflight.git.resolve-base|preflight.git.derive-range|preflight.git.list-ordered-commits|preflight.git.read-material|runner.git.materialize-mirror|runner.git.materialize-workspace|runner.git.verify-snapshot`. A bash command is authorized only by byte-exact string equality, and requires `bash` in `tools`. Unknown keys/values, duplicates, digest mismatch, unavailable requested/effective host tools, and any requested widening fail closed; unused ceiling authority may be unavailable on the current host and there is no fallback grant.

The runtime transforms the first input through native `/skill:code-review` and verifies the complete expanded content against the canonical activation snapshot. The parent submits one atomic V1 `Agent` proposal: `base.revision`, a `standardsMaterials` array that may be empty, an established Spec with non-empty `materials` or a not-established Spec with non-empty `evidence`, and exact per-leg `required` capability subsets. The complete canonical Standards baseline remains mandatory even when `standardsMaterials` is empty. Rejected preflight proposals start no child and may be corrected; acceptance is irreversible and permits exactly one dispatch. Established Spec runs isolated Standards and Spec legs; established no-Spec runs one complete Standards-only leg with no Spec prompt bytes. Accepted construction and semantic-audit evidence retain the parent task and validated capability document’s exact UTF-8 text, length, and digest. Child prompts instead receive deterministic axis-specific instructions and selected materials, plus task identity/digest and the pinned target/base/range/commits; opaque parent-task prose is not indiscriminately copied across axes. Selected material paths are emitted only as escaped JSON data labels alongside their bytes. Each child grant is a task-bound subset of the invocation ceiling. Generic V1 capability documents may omit `bash` or carry an empty command array, but those shapes are insufficient for an accepted code-review recipe. After deriving the range, the recipe requires the invocation ceiling to include `bash` and the canonical three-dot diff command, and every constructed leg must grant `bash` plus that byte-exact command. A leg may additionally grant only verification commands explicitly declared by that leg that are byte-exact members of the task-bound ceiling. Compiled prompt byte lengths and SHA-256 digests are retained for the post-run semantic audit.
