...
**Machine method Skills:** Run `ak-role setup` to install the required original Skills once under `~/.agents/skills`; setup adds links for installed Claude Code and Hermes when needed and leaves any same-name machine Skill untouched. This directory is the sole machine installation source. Roles warn on stdout when their required Skill is missing and continue. Pi receives paths from that directory through native `--skill`. Claude Code receives per-turn native `Skill(name)` permissions. Codex uses native Skill discovery; Hermes ACP and Grok ACP cannot force Skills through their current harnesses. The package does not prepend or strip Skill commands in role requests, or modify host trust or configuration.
