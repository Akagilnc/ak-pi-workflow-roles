...
**Machine method Skills:** Method Skills are installed on the user's machine, not copied or linked from this package. Run `ak-role setup` to install the required original Skills once under `~/.agents/skills`; setup adds links for installed Claude Code and Hermes when needed and leaves any same-name machine Skill untouched. Pi and Codex discover the shared directory. Roles warn on stdout when their required Skill is missing, explain the quality impact, and continue. Pi uses its native `--skill` support (and its slash command for one Skill); Claude Code's print harness receives its native slash command only when a role binds exactly one Skill. Codex receives no synthetic invocation or package-specific restriction and follows its own native Skill discovery. Hermes ACP and Grok ACP cannot force Skills through their current harnesses. The package does not modify host trust or configuration.
