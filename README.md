...
**Forced method skills (#922):** Pi uses `--skill`; Claude uses `--plugin-dir` and its native slash command. Codex discovers the packaged methods through its documented project `.agents/skills` catalog, which is created only when absent and then left in place; existing entries are never replaced, and Codex receives the official `$skill-name` invocation. Hermes ACP and Grok ACP cannot force a Skill call through their current harness interfaces; this package does not invent an adapter, capability probe, typed failure, catalog, or operator prerequisite for them. The package never changes host trust or configuration.
