...
**Forced method skills (#922):** Pi uses `--skill`; Claude uses `--plugin-dir` and its native slash command. Codex and Hermes discover the packaged methods through their documented project `.agents/skills` catalog, which is created only when absent and then left in place; existing entries are never replaced. Before using Hermes in a project, the operator must run `hermes skills trust`; per Hermes help, this allows project skills from `./.hermes/skills` and `./.agents/skills` to load. Grok ACP reports an unsupported-method gap because its invocation surface has no corresponding loader. The package never changes host trust or configuration.
