...
**Forced method skills (#922):** Pi uses `--skill`; Claude uses `--plugin-dir` and its native slash command. Codex and Hermes can discover methods already supplied by a consuming project through their documented `.agents/skills` catalog, but their CLI surfaces cannot load this package’s methods into an arbitrary project; the package therefore reports an unsupported-method gap rather than creating or changing that project. For project-provided Hermes skills, the operator must run `hermes skills trust`; per Hermes help, this allows project skills from `./.hermes/skills` and `./.agents/skills` to load. Grok ACP reports an unsupported-method gap because its invocation surface has no corresponding loader. The package never changes host trust or configuration.
...
# secretariat — rewrite ticket per 票面法 and drive countersign to converged or escalate (#924)
