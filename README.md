...
Codex fast tier: the repo patches pi-ai (`patches/`, applied by every `pnpm install`) with a file-content switch at `~/.pi-codex-fast`. On: `echo "fast_mode = on" > ~/.pi-codex-fast`; off: `echo "fast_mode = off" > ~/.pi-codex-fast` (or delete the file). The whole file must match `fast_mode = on`—missing, empty, `off`, or any other content means stock behavior. The file is re-read on every codex request (never cached), so flipping it takes effect on the next request in already-running processes—no restart, no new shell. Every patched process sharing the same HOME reads the same switch. An explicit `serviceTier` always wins over the switch. `"priority"` costs more than the default tier; pricing reads the injected value downstream, so cost accounting stays honest. When the injection actually happens the patch prints one stderr line per request: `[ak-patch] codex fast: service_tier=priority (switch enabled)`. Existing npm-installed copies (global Pi, older worktrees) are unaffected.
