...
When settlement is accepted and Navigator prepared a recommendation, the **accepted `ak_*_output` toolResult** in the role session carries a top-level `details.navigation` object (`disposition: "recommendation"`, plus `route` when the route changed, `next`, `reason`, `command`) and the same essentials in the toolResult content text. Canonical headless callers that extract only the last `ak_*_output` settlement record therefore see the advice in that one read—no second grep, no sibling file, no extra nesting beyond the receipt fields they already open. Unavailable and intentional silence (human-decision / role-infrastructure) omit `navigation` entirely; the role Receipt remains valid and otherwise unchanged. A sibling top-level `ak-navigator-attendance` custom_message and stdout typed events remain for interactive surfaces; they are not required for the headless extraction path.
