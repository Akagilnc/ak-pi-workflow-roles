...
**Forced method skills (#922):** Pi uses `--skill`; Claude uses `--plugin-dir` and its native slash command. Codex and Hermes use their documented `.agents/skills` discovery when the consuming project supplies that catalog; the package does not create or change an arbitrary consuming project. For project-provided Hermes skills, the operator must run `hermes skills trust`; per Hermes help, this allows project skills from `./.hermes/skills` and `./.agents/skills` to load. Grok ACP reports an unsupported-method gap because its invocation surface has no corresponding loader. The package never changes host trust or configuration.
