...
Codex fast tier: the repo patches pi-ai (`patches/`, applied by every `pnpm install`) so that the presence of `PI_TEMP_FAST_MODE`—even set empty—defaults codex requests to `service_tier: "priority"`; unset keeps stock behavior. Existing npm-installed copies (global Pi, older worktrees) are unaffected.
