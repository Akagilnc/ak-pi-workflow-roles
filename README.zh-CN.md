...
**强制方法 Skill（#922）：** claude／grok 经 `--plugin-dir` 装载随包方法；codex／hermes 经 cwd `.agents/skills` 符号链接指向随包方法树（共享信封缺则创建；绝不覆盖既有 catalog；链接保留为稳定 catalog，避免并发进程互删 loader 路径）。hermes 另需操作员对该项目执行一次 `hermes skills trust`（若席位用 `-p ak-<role>` 配置，trust 也须在同一 profile 下）；工厂不写宿主 trust 配置，未 trust 时响亮失败。
