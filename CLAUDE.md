...
**读取与呈现不设上限**：代码可以读任意字段。交卷工具只负责记录——调几次记几次，不 abort、不封账、不判 sole；终局由宿主自己结束（退出码 / turn 结束）。角色停了之后账本里有什么就呈什么，没有就是 no_receipt 记一笔（#836）。
...
各 ADR 已落定的具体决策，在本仓优先于宪法的通用条文；其中违反宪法者，须绑陛下原话逐字，或能回溯到该原话的指针（起居录路径、源卷 uuid）。**ADR 自身路径不是指针**——ADR 不能引自己当授权。
