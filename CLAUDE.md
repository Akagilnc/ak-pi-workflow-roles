...
各 ADR 已落定的具体决策，在本仓优先于宪法的通用条文；其中违反宪法者，须绑陛下原话逐字，或指针（起居录路径、源卷 uuid、ADR 路径）。
...
没有陛下原话逐字或指针（起居录路径、源卷 uuid、ADR 路径），不得标作陛下 authority。**指针用 uuid，不用行号**——源卷行号会移位，同一 uuid 亦可在卷内重复出现。
