...
**代码对角色交上来的东西没有拒收权，也不得因形状不合中止本局**——数组性、非空、字段全集、跨字段组合、字段名拼写，一概不作拒收理由。**本条只约束代码**：审刑院、大理寺据同样内容打回是它们的职掌，不在禁止之列（角色打回走重交，不掐局）。

**schema 与 prompt 里的字段声明是「要求」，不是「必须」**，不授权代码强制检查。key 名字不必逐字相同，合不合格由审刑院按**读不读得懂**判，读得懂就放行；要紧的是内容是否属实。

**读取与呈现不设上限**：代码可以读任意字段、落账、渲染、对账。本条禁的是「因什么拒收或中止」，不是「能读什么」。代码仍拥有两件事：受理唯一一次交卷，交卷后终止本局。

**记账位**：每份角色输出有且只有一个精确 key 及取值域（如 `judgeStatus`），供落账与呈现，**不是编排控制流**（派谁归调用者，ADR 0010）。其有效性判定仍归 ADR 0040；本条只禁一件——不得因它缺失、拼错或取值域外而中止本局；该情形原样落账、显式呈现「未判」、交调用者。

**不在射程的保留项**（本条是该判据的唯一真源，别处只引用）：① 各 ADR 自述为 ADR 0036 特别理由例外者；② 另有独立 authority 的保留裁定（举例：ADR 0018 激活闸、0023 runtime 行为验证、0041 sole final call）。两支均以各 ADR 自述为准，举例不是穷举。它们属语义或现场事实核验，失败按失败诚实宪法与 ADR 0052 响亮终结。

「只验证必须有的」真源在 ADR 0025 / 0036；删与留之争按 CONTEXT.md「承接者判据」裁。

**下一个读它的是角色，不是代码。** 读不懂的只有一份因形状不合而根本没交出来的判词。
