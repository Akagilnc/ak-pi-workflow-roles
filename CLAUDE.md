...
**生产与统计两个 regime（陛下 2026-08-25 释宪）**：上条禁令的对象是生产代码对不可穷举输入的机械依赖。统计/分析对**已冻结卷宗**不适用该禁令——太史报告层可由 LLM 做语义分类与统计，逐条附卷宗引证即为可核；形式语言命令（shell 等）非散文，不因 LLM 书写而成「自由文本」。太史代码机制维持确定性（ADR 0047/0068 不动）；闸判据仍只认 typed 键。
