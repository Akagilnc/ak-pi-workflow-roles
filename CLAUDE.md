...
**不在本条射程的保留项**：凡各 ADR 自述为 ADR 0036 特别理由保留例外者，以及 ADR 0018 的激活闸、ADR 0037 的实时对象身份与证据绑定、ADR 0041 的 sole final call。**名单以各 ADR 自述为准，施工前逐条复核，本文件不复制名单。** 这些属语义或现场事实核验，非形状校验；其失败按失败诚实宪法与 ADR 0052 走响亮终结，不得静默放行。
