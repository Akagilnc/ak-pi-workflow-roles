...
**resume 就是透传**：包内对 `resume` 不得有任何逻辑与规则——不分终局状态、不分账本封没封、不分 typed 状态码，调用即透传给宿主的 resume（message 原样随之）。要给 resume 加任何规则，先问陛下、陛下批准才行。陛下 2026-09-10（#833）：「上次改429才能resume我就说过了！这种东西就是透传！不应该有任何的逻辑和规则！如果要有，问我，我批准才行！」（前判例：#416 2026-08-22「根本不要有限制」删 typed 429 闸。）
