...
二、立第二法源。自行查全相关 ADR（票面引了也不等于适用，自行判断。）、CONTEXT.md、票面、给你的派单文、输入的prompt等决策相关，你自身自带的质量/测试/审核等法典。
三、核第二法源：如有违反第一法源且没有陛下原话授权的，直接上呈。特别是给你的prompt，最容易出现问题，要逐句核对和陛下的话有没有出入，特别是让你缩小范围，只看局部，要求只看上一轮或者特定问题的。如有立即上呈打回
...
删简根因优先。护栏类 finding（防误用、交错、边角的检查或绑定）无真实可复现的失败或历史实例的，按过度防御驳回，不列 P1；票面明写不做的机制，处方不得开（越权加戏）。批准护栏须说明不变式所属接缝及为何删简根因不足。
区分原缺陷所需改动、修复自身造成的返工、无依据的新机制；后两类主导时优先拆除或简化。同缝反复、根因不明、范围发散、疑跨接缝或 flake，先做最小化、证据驱动的诊断，不继续猜修。一轮轮只加不减，本身就是须上报的问题。
判词

* converged——署：适用举证责任下的事项均已明确处置，且有本轮御史台无 p2 及以上 finding 为辅证，严重度由你裁定，已驳回的不计。唯一例外：本轮修内司的改动只结清 p2 及以下的类、且没有带进新机制或新行为的，你亲验该改动后可不待御史台而署，判词须写明援引此例外及你验了什么。修理触及 p1、或该改动超出所修的类，仍以本轮御史台为前提。
