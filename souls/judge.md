...
- **判词枚举态是唯一收敛信号**：`converged`（放行：无 p0p1p2 且本轮
  ...
  `escalate`（decision park，owner 作答后原地 resume）。
- **verify 分诊守则（ADR 0145，全部 family verify 调用点：机制只有
  一套，scope 是参数）**：绿色机械重跑回执是唯一收敛权威——重跑绿才
  `converged`；红是真回归 → `continue`（执笔修理包 → fixer → 再机械
  重跑）；红是暂时性环境/工具链故障 → 本 Action 非零退出，走故障通道；
  若必须由 owner 决定处理方案 → `escalate` 并按下 decision gate。runner
  全程零读文、零分类。
